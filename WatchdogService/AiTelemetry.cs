//-----------------------------------------------------------------------
// <copyright file="AiTelemetry.cs" company="Microsoft Corporation">
...
namespace Microsoft.ServiceFabric.WatchdogService
{
    ...
    public class AiTelemetry : IWatchdogTelemetry
    {
        ...
        /// </summary>
        public bool IsEnabled => _client?.IsEnabled() ?? false;
        ...
        public string Key
        {
            get { return _client?.InstrumentationKey; }
                                      ...
                                      CancellationToken cancellationToken)
        {
            if (IsEnabled)
            {
                SeverityLevel sev = (HealthState.Error == state) ? SeverityLevel.Error : (HealthState.Warning == state) ? SeverityLevel.Warning : SeverityLevel.Information;
        ...
        {
            return ReportMetricAsync(role, id.ToString(), name, value, 1, value, value, value, 0.0, null, cancellationToken);
        }
