...
namespace Microsoft.ServiceFabric.WatchdogService
{
    ...
    internal class OwinCommunicationListener : ICommunicationListener, IHttpControllerActivator
    {
        ...
        public OwinCommunicationListener(WatchdogService service, ServiceContext serviceContext, string endpointName, string appRoot)
        {
            ...
            if (null == serviceContext) throw new ArgumentNullException(nameof(serviceContext));
            if (string.IsNullOrWhiteSpace(endpointName)) throw new ArgumentException(nameof(endpointName));
            if (string.IsNullOrWhiteSpace(appRoot)) throw new ArgumentException(nameof(appRoot));
        ...
        { 
            ...
            var ci = controllerType.GetConstructor(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null, CallingConventions.HasThis, new[] { typeof(WatchdogService) }, new ParameterModifier[0]);
            if (null != ci)
            {
                return ci.Invoke(new object[1] { _service }) as IHttpController;
            }
